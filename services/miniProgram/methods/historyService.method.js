...
class MeAPI {
		...
		try {
			...
			if (this.config.isSecurity === true) {
				try {
					...
					// console.log('meAPIHeader_response', response);
	...
	async Put(pathUrl, payload, accessToken = '', headers = {}) {
		const result = {
			code: -1,
			data: {},
			original: null,
...
module.exports = new MeAPI({
	url: process.env.url, publicKey: process.env.publicKey, privateKey: process.env.privateKey, isSecurity: true, 'x-api-client': 'app',
});
